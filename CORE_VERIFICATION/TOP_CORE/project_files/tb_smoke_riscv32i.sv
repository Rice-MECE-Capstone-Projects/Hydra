...
module tb_smoke_riscv32i;
    ...
    // -----------------------------------------
    // Golden/reference ALU outputs
    // -----------------------------------------
    golden_alu_t gold_alu;

    // Execute-stage valid signal
    logic alu_valid;

// Branch stage signals
logic        gold_branch_taken;
logic        gold_jump_taken;
logic [31:0] gold_branch_target;
logic        branch_valid;
    ...
    // -----------------------------------------
    // Golden ALU generation
    // -----------------------------------------
    always_comb begin
        gold_alu = golden_alu_compute(
            dut.u_riscv32i_main.Single_Instruction_stage1,
            dut.u_riscv32i_main.operand1_into_exec,
            dut.u_riscv32i_main.operand2_into_exec,
            dut.u_riscv32i_main.imm_stage1,
            dut.u_riscv32i_main.pc_stage_1,
            dut.u_riscv32i_main.rd_stage1
        );
    end

    // Always check ALU outputs
    assign alu_valid = 1'b1;

// branch process and validation
always_comb begin
    gold_branch_taken = gold_alu.branch_inst;
    gold_jump_taken   = gold_alu.jump_inst;
    gold_branch_target = gold_alu.alu_result_2;
end

assign branch_valid =
    gold_alu.branch_inst ||
    gold_alu.jump_inst ||
    dut.u_riscv32i_main.branch_inst_wire ||
    dut.u_riscv32i_main.jump_inst_wire;

    // -----------------------------------------
    // ALU Scoreboard
    // -----------------------------------------
    alu_scoreboard alu_sb_i (
        .clk(clk),
        .valid(alu_valid),

        .pc(dut.u_riscv32i_main.pc_stage_1),
        .instruction(dut.u_riscv32i_main.instruction_stage_1),
        .single_instruction(dut.u_riscv32i_main.Single_Instruction_stage1),

        // DUT outputs
        .dut_alu_result_1(dut.u_riscv32i_main.alu_result_1),
        .dut_alu_result_2(dut.u_riscv32i_main.alu_result_2),
        .dut_branch_inst(dut.u_riscv32i_main.branch_inst_wire),
        .dut_jump_inst(dut.u_riscv32i_main.jump_inst_wire),
        .dut_write_reg(dut.u_riscv32i_main.write_reg_file_wire),

        // Golden outputs
        .gold_alu_result_1(gold_alu.alu_result_1),
        .gold_alu_result_2(gold_alu.alu_result_2),
        .gold_branch_inst(gold_alu.branch_inst),
        .gold_jump_inst(gold_alu.jump_inst),
        .gold_write_reg(gold_alu.write_reg_file)
    );

// Branch scoreboard

branch_scoreboard branch_sb_i (
    .clk(clk),
    .valid(branch_valid),
    ...
    .dut_branch_taken(dut.u_riscv32i_main.branch_inst_wire),
    .dut_jump_taken(dut.u_riscv32i_main.jump_inst_wire),
    .dut_target_pc(dut.u_riscv32i_main.alu_result_2),

    .gold_branch_taken(gold_branch_taken),
    .gold_jump_taken(gold_jump_taken),
    .gold_target_pc(gold_branch_target)
);
    ...
    initial begin
        ...
        alu_sb_i.print_summary();
	branch_sb_i.print_summary();
        $finish;
    end
