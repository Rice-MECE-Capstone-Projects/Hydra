...
module dataMem #(parameter mem_size = 4096) (
    ...
    always @(posedge clk) begin
        if (!reset && mem_write)
            $display("[DMEM] t=%0t WRITE addr=%h data=%h", $time, mem_addr, mem_wdata);
        if (!reset && mem_read)
            $display("[DMEM] t=%0t READ  addr=%h → %h", $time, mem_addr, DMEM[word_address]);
    end
endmodule