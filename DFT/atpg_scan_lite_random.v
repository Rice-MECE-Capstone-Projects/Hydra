...
localparam NUM_RANDOM_PATTERNS = 5;
...
// --------------------------------------------------
// test one fault with deterministic + random patterns
// only for pipeReg0/1/2 focused improvement
// --------------------------------------------------
task test_one_fault;
    input integer target_bit;
    input         fault_type;   // 0=SA0, 1=SA1
    ...
    begin
        ...
        if (fault_type == 1) begin
            // SA1 deterministic patterns
            bg = gen_pattern(0);  // all-0
            run_one_pattern_for_fault(bg, target_bit, fault_type, det);
            if (det) found = 1;

            if (!found) begin
                bg = gen_pattern(2);  // alt01
                run_one_pattern_for_fault(bg, target_bit, fault_type, det);
                if (det) found = 1;
            end

            if (!found) begin
                bg = gen_pattern(3);  // alt10
                run_one_pattern_for_fault(bg, target_bit, fault_type, det);
                if (det) found = 1;
            end
        end
        else begin
            // SA0 deterministic patterns
            bg = gen_pattern(1);  // all-1
            run_one_pattern_for_fault(bg, target_bit, fault_type, det);
            if (det) found = 1;
        ...
        // random patterns
        if (!found) begin
            for (rp = 0; rp < NUM_RANDOM_PATTERNS; rp = rp + 1) begin
...
// --------------------------------------------------
// main flow
// focused on pipeReg0/1/2 only
// --------------------------------------------------
integer b;

initial begin
    $display("=== ATPG Scan Lite Start ===");
    $display("Focused improvement on pipeReg0/1/2");
    $display("Patterns used: all-0, all-1, alt01, alt10, + %0d random patterns", NUM_RANDOM_PATTERNS);
    ...
    $display("\n========================================");
    $display("=== ATPG Scan Lite Results (pipeReg0/1/2 focused) ===");
