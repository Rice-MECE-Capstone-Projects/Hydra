...
localparam NUM_ITER  = 3;
localparam RAND_PER_ITER = 5;
...
// --------------------------------------------------
// random pattern
// note: Verilog function needs at least one input
// --------------------------------------------------
...
// --------------------------------------------------
// test one fault using:
//   deterministic patterns first
//   then automated multi-iteration random patterns
// --------------------------------------------------
task test_one_fault;
    input integer target_bit;
    input         fault_type;   // 0=SA0, 1=SA1
    ...
    begin
        ...
        // automated multiple random iterations
        if (!found) begin
            for (iter = 0; iter < NUM_ITER; iter = iter + 1) begin
                for (rp = 0; rp < RAND_PER_ITER; rp = rp + 1) begin
...
// Sampling:
//   pipeReg0: keep dense
//   pipeReg1: every 20 bits
//   pipeReg2: every 20 bits
// pipeReg3 excluded
// --------------------------------------------------
integer b;
...
initial begin
    ...
    $display("Automate testing and run multiple iterations (pipeReg0/1/2 only)");
    $display("Patterns used: all-0, all-1, alt01, alt10");
    $display("Random strategy: %0d iterations x %0d random patterns each",
             NUM_ITER, RAND_PER_ITER);
    ...
    $display("\n========================================");
    $display("=== ATPG Scan Lite Results (multiple iterations on pipeReg0/1/2) ===");
