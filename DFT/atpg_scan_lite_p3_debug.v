`timescale 1ns/1ps

module atpg_scan_lite;

reg  clk, reset, enable_design, scan_en, scan_in;
wire scan_out;

// active DUT-side stimulus
reg [31:0] tb_data_mem_doutb;
reg [31:0] tb_ins_data_rdata_i;
reg        tb_ins_data_rvalid_i;
reg        tb_ins_data_gnt_i;

riscv32i_main dut (
    .clk(clk), .reset(reset),
    .enable_design(enable_design),
    .scan_en(scan_en),
    .scan_in(scan_in),
    .scan_out(scan_out),
    .Cycle_count(32'b0),
    .memory_offset(32'b0),
    .initial_pc_i(32'b0),
    .final_value(),
    .data_mem_clkb(), .data_mem_enb(), .data_mem_rstb(),
    .data_mem_web(), .data_mem_addrb(), .data_mem_dinb(),
    .data_mem_rstb_busy(1'b0),
    .data_mem_doutb(tb_data_mem_doutb),
    .ins_data_req_o(), .ins_data_addr_o(), .ins_data_we_o(),
    .ins_data_be_o(), .ins_data_wdata_o(),
    .ins_data_rdata_i(tb_ins_data_rdata_i),
    .ins_data_rvalid_i(tb_ins_data_rvalid_i),
    .ins_data_gnt_i(tb_ins_data_gnt_i)
);

initial clk = 0;
always #5 clk = ~clk;

localparam CHAIN_LEN   = 1604;
localparam TARGET_BIT  = 1161;   // 你可以换成 1171 / 1181 再试
localparam FAULT_TYPE  = 1'b0;   // 0 = SA0, 1 = SA1

reg [CHAIN_LEN-1:0] golden_out, faulty_out;
reg [CHAIN_LEN-1:0] bg_pattern;
reg [CHAIN_LEN-1:0] faulty_pattern;

// --------------------------------------------------
// reset
// --------------------------------------------------
task do_reset;
begin
    reset = 1;
    scan_en = 0;
    enable_design = 0;
    scan_in = 0;
    @(posedge clk); #1;
    @(posedge clk); #1;
    reset = 0; #1;
end
endtask

// --------------------------------------------------
// shift-in
// --------------------------------------------------
task shift_in;
input [CHAIN_LEN-1:0] pattern;
integer i;
begin
    scan_en = 1;
    enable_design = 0;
    for (i = CHAIN_LEN-1; i >= 0; i = i - 1) begin
        scan_in = pattern[i];
        @(posedge clk); #1;
    end
    scan_en = 0;
    scan_in = 0;
end
endtask

// --------------------------------------------------
// 1-cycle capture
// --------------------------------------------------
task do_capture_debug;
begin
    $display("--------------------------------------------------");
    $display("Before capture:");
    $display("  pipeReg3                = %h", dut.pipeReg3);
    $display("  pipeReg3_wire           = %h", dut.pipeReg3_wire);
    $display("  stage3_MEM_valid        = %b", dut.stage3_MEM_valid);
    $display("  stage2_EXEC_valid       = %b", dut.stage2_EXEC_valid);
    $display("  enable_design           = %b", dut.enable_design);
    $display("  delete_reg1_reg2_reg    = %b", dut.delete_reg1_reg2_reg);
    $display("  load_into_reg           = %b", dut.load_into_reg);
    $display("  write_reg_file_wire_s2  = %b", dut.write_reg_file_wire_stage2);

    scan_en = 0;
    enable_design = 1;
    @(posedge clk); #1;
    enable_design = 0;

    $display("After capture:");
    $display("  pipeReg3                = %h", dut.pipeReg3);
    $display("  pipeReg3_wire           = %h", dut.pipeReg3_wire);
    $display("  stage3_MEM_valid        = %b", dut.stage3_MEM_valid);
    $display("  stage2_EXEC_valid       = %b", dut.stage2_EXEC_valid);
    $display("  enable_design           = %b", dut.enable_design);
    $display("  delete_reg1_reg2_reg    = %b", dut.delete_reg1_reg2_reg);
    $display("  load_into_reg           = %b", dut.load_into_reg);
    $display("  write_reg_file_wire_s2  = %b", dut.write_reg_file_wire_stage2);
    $display("--------------------------------------------------");
end
endtask

// --------------------------------------------------
// shift-out
// --------------------------------------------------
task shift_out;
output [CHAIN_LEN-1:0] result;
integer i;
begin
    scan_en = 1;
    enable_design = 0;
    scan_in = 0;
    for (i = CHAIN_LEN-1; i >= 0; i = i - 1) begin
        @(posedge clk); #1;
        result[i] = scan_out;
    end
    scan_en = 0;
end
endtask

// --------------------------------------------------
// simple deterministic pattern
// use all-1 for SA0, all-0 for SA1
// --------------------------------------------------
task make_background_pattern;
input fault_type;
output [CHAIN_LEN-1:0] pattern;
begin
    if (fault_type == 1'b1)
        pattern = {CHAIN_LEN{1'b0}}; // SA1 -> start from all-0
    else
        pattern = {CHAIN_LEN{1'b1}}; // SA0 -> start from all-1
end
endtask

// --------------------------------------------------
// main diagnostic flow
// --------------------------------------------------
initial begin
    $display("=== pipeReg3 root-cause debug testbench ===");
    $display("TARGET_BIT = %0d, FAULT_TYPE = SA%0d", TARGET_BIT, FAULT_TYPE);

    tb_data_mem_doutb    = 32'hA5A5_5A5A;
    tb_ins_data_rdata_i  = 32'h1234_5678;
    tb_ins_data_rvalid_i = 1'b1;
    tb_ins_data_gnt_i    = 1'b1;

    clk = 0;
    reset = 1;
    scan_en = 0;
    enable_design = 0;
    scan_in = 0;
    #20;

    // --------------------------
    // Build background / faulty pattern
    // --------------------------
    make_background_pattern(FAULT_TYPE, bg_pattern);
    faulty_pattern = bg_pattern;
    faulty_pattern[TARGET_BIT] = FAULT_TYPE ? 1'b1 : 1'b0;

    // --------------------------
    // Golden run
    // --------------------------
    $display("\n=== GOLDEN RUN ===");
    do_reset;
    shift_in(bg_pattern);

    $display("Golden after shift_in:");
    $display("  pipeReg3 = %h", dut.pipeReg3);
    $display("  bit[%0d] = %b", TARGET_BIT, dut.pipeReg3[TARGET_BIT-1091]);

    do_capture_debug;

    $display("Golden before shift_out:");
    $display("  pipeReg3 = %h", dut.pipeReg3);
    shift_out(golden_out);

    // --------------------------
    // Faulty run
    // --------------------------
    $display("\n=== FAULTY RUN ===");
    do_reset;
    shift_in(faulty_pattern);

    $display("Faulty after shift_in:");
    $display("  pipeReg3 = %h", dut.pipeReg3);
    $display("  bit[%0d] = %b", TARGET_BIT, dut.pipeReg3[TARGET_BIT-1091]);

    do_capture_debug;

    $display("Faulty before shift_out:");
    $display("  pipeReg3 = %h", dut.pipeReg3);
    shift_out(faulty_out);

    // --------------------------
    // Compare
    // --------------------------
    $display("\n=== COMPARISON ===");
    $display("golden_out = %h", golden_out);
    $display("faulty_out = %h", faulty_out);

    if (golden_out !== faulty_out)
        $display("RESULT: DETECTED");
    else
        $display("RESULT: MISSED");

    $finish;
end

endmodule